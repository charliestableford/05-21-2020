...
//task 1
let primaryLogo = document.querySelector('.primary-logo');
window.onload = (event) => {
...
This test would run for two weeks. Important to note that every test that I run will run for the entrie time laid as out to make sure it is viable. 
I would also want this test to be statistically signifigant. In order to see this I would want to see a 5% increase in folks heading to the checkout. 
    Population: Our population are folks that have added something to their cart.
    ...
    I would use a sample size caluculator to measure the sample size needed. This will take the baseline conversion rate (I would want to set this at 5%). I would want the statistical signifgance to be high 90%-95%. If there was a baseline conversion rate of 5% and a mionimum detectable effect of 1% we will need a sample size f 12,866. If, based on the past statistics of the website (as the graph shown in the test does not show time), the number needed to be tested was to high I would decrease the statistcal power and increase the minimum detectable effect - allowing for more error but a smaller population test size. 
